# 🏥 MediFlow AI — Smart Hospital Queue Prediction System
...
### 🔹 Terminal 2: Patient Web App (React + Vite + TailwindCSS)
...
### 🔹 Terminal 3: Staff & Hospital Admin Dashboard (React + Vite)
...
|---|---|---|---|
| **Patient** | [http://localhost:3000/login](http://localhost:3000/login) | `laxuman.patient@mediflow.ai` *(or any registered email/phone)* | `Patient@123` |
...
### 🏥 Staff & Doctor Dashboard (`http://localhost:5173`)
...
```
[1. Patient Books Slot] ──> [2. AI Calculates Wait & Departure] ──> [3. Staff Sees Live Queue]
                                                                             │
[5. Queue Logged to DB] <── [4. Doctor Calls / Emergency Bumps] <────────────┘
```
...
4. Show the **Digital Pass Modal** generated with real token (e.g. `OPD-031`) and QR code.
...
2. Explain how the system combines:
   ...
   - **Travel Engine (Distance & Traffic)**: Calculates travel time from patient's location.
3. Show the **"Leave Home in X Minutes"** recommendation countdown.

### Scene 3: Staff Dashboard Live Queue Management
1. Open **Staff Dashboard** in another tab at [http://localhost:5173/queue](http://localhost:5173/queue).
2. Show that the patient booked in Scene 1 is already visible in the live table under **Dr. Sneha Patil**!
3. Filter by **Department: Dermatology** or **Doctor: Dr. Sneha Patil**.
